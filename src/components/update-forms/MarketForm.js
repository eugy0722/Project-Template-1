...
function Formulary() {
  ...
  const onSubmit = (data) => {
    ...
    if (!(inputs.name || inputs.geo)) alert('Por favor, insira um campo para actualizar');
    else
      axios
      ...
      <form noValidate onSubmit={handleSubmit(onSubmit)}>
        <Grid container spacing={3}>
          <Grid item xs={12} md={6}>
            <Stack spacing={1}>
              <InputLabel htmlFor="name-market">Novo Nome Do Mercado</InputLabel>
              <OutlinedInput
                fullWidth
              ...
              <FormHelperText error id="helper-text-name-market">
                {errors.name?.type === 'minLength' && <p role="alert">Um nome deve ter no mínimo 4 letras e no máximo 40 letras</p>}
              </FormHelperText>
            </Stack>
          </Grid>
          <Grid item xs={12} md={6}>
            <Stack spacing={1}>
              <InputLabel htmlFor="geo-market">Nova Localização do mercado</InputLabel>
              <OutlinedInput
                fullWidth
              ...
              />
              <FormHelperText error id="helper-text-geo-market"></FormHelperText>
            </Stack>
          </Grid>
          <Grid item xs={12}>
            <AnimateButton>

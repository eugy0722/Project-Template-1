...
function Formulary() {
  ...
  const onSubmit = (data) => {
    ...
    if (!(inputs.name || inputs.type)) alert('Por favor, insira um campo para actualizar');
    else
      axios
      ...
      <form noValidate onSubmit={handleSubmit(onSubmit)}>
        <Grid container spacing={3}>
          <Grid item xs={12} md={6}>
            <Stack spacing={1}>
              <InputLabel htmlFor="name-business">Nome do Novo Negócio*</InputLabel>
              <OutlinedInput
              ...
              <FormHelperText error id="helper-text-name-business">
                {errors.name?.type === 'maxLength' && <p role="alert">O nome deve ter no maximo 40 letras</p>}
              </FormHelperText>
            </Stack>
          </Grid>
          <Grid item xs={12} md={6}>
            <Stack spacing={1}>
              ...
              <Select labelId="demo-simple-select-label" id="type-signup" name="type" label="Type" {...register('type', {})}>
                <MenuItem value={'Producto'}>Producto</MenuItem>
                <MenuItem value={'Servicos'}>Serviços</MenuItem>
              </Select>
              <FormHelperText error id="helper-text-type-signup"></FormHelperText>
            </Stack>
          </Grid>
          <Grid item xs={12}>
            <AnimateButton>
